...
<div className="w-full grid md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-8 md:px-8 px-4 pb-12 items-center">
      ...
      {CollectionData.map((collection, blockIndex) => (
          <div key={blockIndex} className="border-[1px]  border-border rounded-lg  px-4 w-full">
          <div className="pt-8 pb-4 flex place-items-center space-x-2 border-b-[1px]  border-border">
           {collection.Icon} 
            <h1 className="text-[20px] text-primaryText font-[700]">{collection.title}</h1>
          </div>
          <div className="space-y-4 pt-4">
            {collection.texts.map((text, textIndex) => (
              <div key={textIndex} className="flex place-items-center space-x-1">
                ...
                <h3 className="opacity-[1] text-[14px] hover:text-gray-950 font-[500] text-secondaryText">{text}</h3>
              </div>
            ))}
          </div>
          <div className="flex place-items-center space-x-2 text-[#4831d4] border-t-[1px] border-border mt-4 pt-2 mb-4">
